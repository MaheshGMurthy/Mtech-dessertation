library(readr)
phenotype<-read.delim("C:/Users/manum/Desktop/skin.txt", header=TRUE, sep = "\t")
...
write.csv(merge1, "combinedata_skin.csv", row.names = FALSE)


#filter the males and females
males<- subset(merge1, SEX=="1")
write.csv(males, "males_skin.csv", row.names = FALSE)
females<-subset(merge1, SEX=="2")
write.csv(females, "females.csv", row.names = FALSE)




# Function to run regression and calculate q-values
run_regression <- function(females) {
  ...
  for (gene in unique(females$name)) {
    # Filter data for the current gene
    gene_data <- subset(females, name == gene)
...
write.csv(male_results, "male_regression_results_skin.csv", row.names = FALSE)
write.csv(female_results, "female_regression_results_skin.csv", row.names = FALSE)
...
age_regression_results$P_Value <- as.numeric(age_regression_results$P_Value)

# Create the histogram
age_skin <- ggplot(age_regression_results, aes(x = P_Value)) +
  geom_histogram(binwidth = 0.05, fill = "red", color = "black", alpha = 0.7) +
  theme_minimal() +
  labs(title = "Histogram of P-Values - skin tissue",
       x = "P-Value",
       y = "Frequency") +
...
age_regression_results$logP <- -log10(age_regression_results$P_Value)
...
library(qvalue)
pvalue<-age_regression_results$P_Value

# Calculate q-values from p-values
qobj <- qvalue(p = age_regression_results$P_Value)

# Add the q-values to your data frame
age_regression_results$q_value <- qobj$qvalues

# View the updated data frame
head(age_regression_results)
...
age_regression_results$Significant <- age_regression_results$q_value < q_value_threshold

# View the updated data frame
print(age_regression_results)

# Filter the data frame to include only significant genes
significant_genes <- age_regression_results[age_regression_results$Significant == TRUE, ]

# Optionally, save the significant genes to a new CSV file
write.csv(significant_genes, "significant_genes_qvalues_skin.csv", row.names = FALSE)
...
merge2<- merge(age_regression_results, gencode26, by.x="Gene", by.y= "name")

write.csv(merge2, "age_regression_skin2.csv", row.names = FALSE)
...
manhattan(merge2, chr = "chromosome", bp = "position", p = "P_Value", snp = "Gene",
          ...
          col = c("blue4", "yellow2"), chrlabs = NULL, main = "Manhattan Plot-skin tissue",
          ylim = c(0, max(-log10(merge2$P_Value) + 2)))


#to adjust the number of hits
# 1. reduce the p value threshold
# 2. multiple testing correction - Bonferroni correction

#Lets try p value threshold

p_value_threshold<- 0.01
significant_hits <- subset(age_regression_skin2, P_Value < p_value_threshold)
#got 8501 hits

#lets try bonferroni correction
bonferroni_threshold <- 0.05 / length(age_regression_skin2$P_Value)
bon_significant<- subset(age_regression_skin2, P_Value < bonferroni_threshold)

write.csv(bon_significant, "Bon_significant_skin.csv", row.names = FALSE)
...
total_gene_list <- age_regression_skin2$Gene
significant_genes_list <- Bon_significant_skin$Gene
gwas_association_data <- gwas.skin
...
ggplot(top20_data, aes(x = log_odds_ratio, y = reorder(Description, log_odds_ratio), fill = neg_log_pvalue)) +
  ...
  labs(title = "Top 20 KEGG Pathway Enrichment in Skin tissue", x = "Log Odds Ratio", y = "Pathway", fill = "-Log10(p-value)") +
  theme_minimal() +
